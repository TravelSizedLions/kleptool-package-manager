...
program.command('add')
  ...
  .action(errorBoundary(async (...args: unknown[]) => {
    const [url, options] = args as [string, { version?: string, rename?: string, dev?: boolean, extract?: string, to?: string }];
    ...
    klep.saveDeps()
    console.log(`Added dependency ${name}@${v} to ${options.dev ? 'development dependencies' : 'dependencies'}`)
  }));
