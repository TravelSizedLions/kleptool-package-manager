...
fn parse_to_ast_tree(input: ParseInput) -> Result<AstTree, Box<dyn std::error::Error>> {
    debug_log(&format!("Parsing {} code ({} chars)", input.language, input.source_code.len()));
    ...
    let dummy_tree = AstTree {
        root: AstNode {
            ...
            value: None,
            location: Location { line: 1, column: 1, byte_offset: 0 },
            children: vec![
                AstNode {
    ...
    debug_log(&format!("Successfully parsed into {} nodes", dummy_tree.metadata.total_nodes));
    Ok(dummy_tree)
}
