...
use crate::types::{
    MutationContext, MutationType, ParsedFile,
};
...
impl MutationEngine {
    ...
    pub fn generate_ast_mutations(&self, parsed_file: &ParsedFile) -> Vec<crate::types::Mutation> {
        ...
        let candidates = self.parser.extract_mutation_candidates(&parsed_file.ast, &parsed_file.stripped_content);
...
"#)?;
        ...
        for mutation in &mutations {
            println!("  {} -> {} ({})", mutation.original, mutation.mutated, mutation.mutation_type.description());
        }
...
"#)?;
        ...
        let has_boolean = mutations.iter().any(|m| matches!(m.mutation_type, MutationType::BooleanLiteral));
        let has_number = mutations.iter().any(|m| matches!(m.mutation_type, MutationType::NumberLiteral));
        let has_string = mutations.iter().any(|m| matches!(m.mutation_type, MutationType::StringLiteral));
