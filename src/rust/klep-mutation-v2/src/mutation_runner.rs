...
use crate::cache::{MutationCache, CachedTestResult};
use crate::file_safety::{SafeFileManager, SafetyGuard};
use crate::types::{Mutation, MutationResult, KillType};
...
impl MutationRunner {
    ...
    pub async fn run_mutations_safely(
        ...
        let mut files_to_prepare: Vec<_> = mutations.iter()
            .map(|m| &m.file)
            .collect();
        ...
        println!("🛡️  File safety initialized for {} files", files_to_prepare.len());
        
        // Run mutations in parallel with safety guarantees
        let results = self.run_mutations_parallel(mutations, verbose, prepared_manager).await?;
        ...
        let mutation_futures: FuturesUnordered<_> = mutations
            ...
            .map(|mutation| {
                ...
                async move {
                    ...
                    let result = self.run_single_mutation_safely(mutation, file_manager.as_ref()).await;
    ...
    async fn run_single_mutation_safely(
        ...
        let content_hash = self.cache.get_content_hash(&mutation.file, &mutated_content);
        ...
        let restoration_token = file_manager.apply_mutation_temporarily(&mutation.file, &mutated_content)?;
        ...
        file_manager.restore_file(restoration_token)
            .with_context(|| format!("CRITICAL: Failed to restore file after mutation: {}", mutation.file.display()))?;
        ...
        self.cache.cache_mutation_result(content_hash, cached_result);
    ...
    fn apply_mutation_to_content(
        ...
        let temp_copy_path = file_manager.get_temp_copy(&mutation.file)
        ...
        if start_byte > content.len() || end_byte > content.len() || start_byte > end_byte {
            anyhow::bail!("Invalid mutation span for file: {}", mutation.file.display());
        }
    ...
    async fn run_test_with_timeout(&self) -> Result<String, String> {
        ...
        match tokio::time::timeout(timeout_duration, test_future).await {
            ...
            Ok(Err(e)) => {
                Err(format!("Failed to execute test command: {}", e))
            }
            Err(_) => {
                Err("Test execution timed out after 30 seconds".to_string())
            }
        }
    }
