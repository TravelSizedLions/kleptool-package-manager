...
impl SafeFileManager {
    pub fn new() -> Result<Self> {
        let temp_dir = TempDir::new()
            .context("Failed to create temporary directory for safe file operations")?;
        
        println!("🛡️  Initialized safe file manager with temp dir: {:?}", temp_dir.path());
    ...
    pub fn prepare_file_for_mutation(&mut self, file_path: &Path) -> Result<PathBuf> {
        ...
        let temp_file_name = format!("mutation_{}_{}", 
            file_id.simple(), 
            file_path.file_name()
                .and_then(|n| n.to_str())
        ...
        inner.temp_copies.insert(file_path.to_path_buf(), temp_file_path.clone());
        inner.original_contents.insert(file_path.to_path_buf(), original_content);
    ...
    pub fn apply_mutation_temporarily(&self, file_path: &Path, mutated_content: &str) -> Result<RestorationToken> {
        let inner = self.inner.lock().unwrap();
        
        // Verify we have the original content backed up
        let original_content = inner.original_contents.get(file_path)
    ...
    pub fn restore_file(&self, token: RestorationToken) -> Result<()> {
        fs::write(&token.file_path, &token.original_content)
            .with_context(|| format!("CRITICAL: Failed to restore file: {}", token.file_path.display()))?;
    ...
    pub fn emergency_restore_all(&self) -> Result<()> {
        ...
        if !errors.is_empty() {
            anyhow::bail!("CRITICAL ERRORS during emergency restoration:\n{}", errors.join("\n"));
        }
...
impl Drop for RestorationToken {
    fn drop(&mut self) {
        ...
        if let Err(e) = fs::write(&self.file_path, &self.original_content) {
            eprintln!("🚨 EMERGENCY: Failed to auto-restore file during Drop: {} - {}", 
                self.file_path.display(), e);
        } else {
            println!("🛡️  Auto-restored file during cleanup: {}", self.file_path.display());
        }
    }
}
    ...
    fn drop(&mut self) {
        if std::thread::panicking() {
            ...
            if let Err(e) = self.file_manager.emergency_restore_all() {
                eprintln!("💥 CRITICAL: Emergency restoration failed during panic: {}", e);
            }
        }
    }
}
