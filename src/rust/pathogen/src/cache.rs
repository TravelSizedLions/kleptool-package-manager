...
impl MutationCache {
  pub fn new() -> Self {
    Self {
      baseline_cache: Arc::new(Mutex::new(LruCache::new(NonZeroUsize::new(100).unwrap()))),
  ...
  pub fn get_content_hash(&self, file: &PathBuf, content: &str) -> ContentHash {
    ...
    self
      .content_hash_cache
      .insert(file.clone(), (hash.clone(), now));
    hash
  }
  ...
  pub fn cleanup_stale_entries(&self) {
    ...
    self
      .mutation_cache
      .retain(|_, result| !result.is_stale(self.mutation_ttl));

    // Clean content hash cache (keep for 10 minutes)
    let content_ttl = Duration::from_secs(600);
    self
      .content_hash_cache
      .retain(|_, (_, cached_at)| cached_at.elapsed().unwrap_or(Duration::MAX) < content_ttl);
  }
